...
public class UserInteractions extends EmployeeInteractions {

    public UserInteractions(DataAccessInterface daoObject) {
        super(daoObject);
    }
    ...
    public String createHelpRequest(String jsonFromApi) {
        ...
        DatabaseEntity helpRequestUser = new DatabaseEntity(helpRequestMap);
        helpRequestUser.sanitizeFromApi();
        if(helpRequestUser.newRowObject.containsKey("description")){
            if (helpRequestUser.newRowObject.get("description").length() > 250){
                throw new MalformedObjectException("Please enter less than 250 characters in the description box");
            } else if (daoObject.selectObjectsDb(helpRequestUser.returnSqlForSelectByEmployeeId()).length < 1){
                HashMap<String, String> databaseResponse =
                        daoObject.insertObjectDb(helpRequestUser.returnSqlForInsertOne()).newRowObject;
